REM FastInstall 3.0 School edition
...
echo.
echo # Premi 'N' per saltare...

CHOICE /C SN /T 4 /D S
IF ERRORLEVEL 255 goto prginstall
IF ERRORLEVEL 2 goto prginstall
IF ERRORLEVEL 1 (CALL %fipath%choco-install.bat)
...
echo.
choco install 7zip vlc pdfxchangeeditor audacity -y
choco install notepadplusplus -y
choco install wiztree -y
choco install zoom -y
choco install googlechrome -y
choco install irfanview irfanviewplugins -y
choco install picotorrent aria2 libreoffice -y
choco install open-shell naps2 gimp -y
choco install cdburnerxp veeam-agent skype -y
